...
**UIWidget taint (TextWithState + HorizontalCurrencies + ItemDisplay + StatusBar + Spacer):** `UIWidgetTemplateTextWithStateMixin:Setup` and `UIWidgetTemplateHorizontalCurrenciesMixin:Setup` call `GetStringHeight()`/`GetHeight()` which return secret numbers in tainted AreaPOI tooltip contexts. Fix: wrap each mixin's `Setup` with `pcall` to suppress taint errors. Item display widgets are trickier: `UIWidgetTemplateItemDisplayMixin:Setup` resumes through Blizzard's internal `xpcall`, so an outer `pcall` still lets BugGrabber record the secret-number error first. Temporarily replace the widget's `ItemName` / `InfoText` dimension getters with safe wrappers during setup, then restore them. Widgets can also fail after setup in `DefaultWidgetLayout -> GetUnscaledFrameRect` when `frame:GetRect()` or `frame:GetScaledRect()` returns secret values; install instance-level guards on widget frames and wrap `UIWidgetContainerMixin:CreateWidget` / `ProcessWidget` / `UpdateWidgetLayout` so guards are applied before Blizzard runs layout. Do not wrap Blizzard's global layout or tooltip functions. See `registerTooltipHooks()` in `EnhanceQoLTooltip.lua`.

**LFG declined applications:** `C_LFGList.CancelApplication(resultID)` alone does not immediately make a declined search result selectable/sign-up-able again; Blizzard often keeps the row in the red `Declined` state until the search is refreshed or the window is reopened. For one-click requeue, cancel the application, trigger the current search again, then retry selecting/signing up for the same `resultID` for a short window. See the `LFGListSearchEntry_OnClick` hook in `EnhanceQoL.lua`.

**Nameplate refresh taint:** Do not hook `AurasFrame.RefreshAuras`, `RefreshLossOfControl`, `BuffFrame.UpdateBuffs`, aura pool resetters, `CompactUnitFrame_UpdateAll`, or `CompactUnitFrame_UpdateHealthColor` to modify default nameplates. Also do not call `CompactUnitFrame_UpdateHealthColor` directly from addon code. On current clients, these patterns can taint `Blizzard_NamePlateAuras.lua` while it handles secret aura fields (`isStealable`, aura instance IDs), and can taint `CompactUnitFrame_UpdateHealPrediction` / `TextStatusBar` while they compare secret health values. For default nameplate mob colors, listen to nameplate/unit events and defer changes with `C_Timer.After(0)`. For default nameplate aura click-through, do not touch Blizzard's aura frames or aura item buttons at all: even deferred `SetMouseClickEnabled` calls on active aura buttons taint the next Blizzard aura refresh.
