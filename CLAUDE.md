...
**UIWidget taint (TextWithState + HorizontalCurrencies + ItemDisplay):** `UIWidgetTemplateTextWithStateMixin:Setup` and `UIWidgetTemplateHorizontalCurrenciesMixin:Setup` call `GetStringHeight()`/`GetHeight()` which return secret numbers in tainted AreaPOI tooltip contexts. Fix: wrap each mixin's `Setup` with `pcall` to suppress taint errors. Item display widgets are trickier: `UIWidgetTemplateItemDisplayMixin:Setup` resumes through Blizzard's internal `xpcall`, so an outer `pcall` still lets BugGrabber record the secret-number error first. Temporarily replace the widget's `ItemName` / `InfoText` dimension getters with safe wrappers during setup, then restore them. See `registerTooltipHooks()` in `EnhanceQoLTooltip.lua`.
