...
**UIWidget taint (TextWithState + HorizontalCurrencies + ItemDisplay + StatusBar):** `UIWidgetTemplateTextWithStateMixin:Setup` and `UIWidgetTemplateHorizontalCurrenciesMixin:Setup` call `GetStringHeight()`/`GetHeight()` which return secret numbers in tainted AreaPOI tooltip contexts. Fix: wrap each mixin's `Setup` with `pcall` to suppress taint errors. Item display widgets are trickier: `UIWidgetTemplateItemDisplayMixin:Setup` resumes through Blizzard's internal `xpcall`, so an outer `pcall` still lets BugGrabber record the secret-number error first. Temporarily replace the widget's `ItemName` / `InfoText` dimension getters with safe wrappers during setup, then restore them. StatusBar widgets can fail after setup in `DefaultWidgetLayout -> GetUnscaledFrameRect` when `frame:GetRect()` returns secret values; install an instance-level `GetRect` guard on `UIWidgetTemplateStatusBarMixin` frames rather than wrapping Blizzard's global layout or tooltip functions. See `registerTooltipHooks()` in `EnhanceQoLTooltip.lua`.
