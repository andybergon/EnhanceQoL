...
**UIWidget TextWithState taint:** `UIWidgetTemplateTextWithStateMixin:Setup` calls `self.Text:GetStringHeight()` which returns a secret number in tainted execution contexts. The subsequent arithmetic (`textHeight - 1`) errors. Fix: wrap the mixin's `Setup` with `pcall` to suppress taint errors. The widget displays with its template default height (text content is unaffected since `SetText` runs before `GetStringHeight`). See `registerTooltipHooks()` in `EnhanceQoLTooltip.lua`.
