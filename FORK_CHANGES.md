...
| Druid Symbiotic Relationship raid reminder fix | Fork-only | — | `main` | Tracks missing Symbiotic Relationship in raids as well as parties; avoids cast-spell range checks because shapeshift forms can make them report false |
...
**Druid Symbiotic Relationship:** The cast/known spell is 474750. The aura to check on the druid player is 474754. The reminder should evaluate in any non-solo group, including raids. Do not use 474750 for nearby-only range checks: Symbiotic Relationship cannot be cast while shapeshifted, so Blizzard's spell-range API can report false even when group members are valid reminder targets.
