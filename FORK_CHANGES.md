...
| LFG one-click apply after decline | Fork-only | — | [`4922f0ce`](https://github.com/andybergon/EnhanceQoL/commit/4922f0ce) (2026-05-02) | Selects the clicked listing before checking Sign Up button state, so a declined selected listing no longer blocks applying to a different key |
| UIWidget Setup taint fixes | Fork-only | — | `main` | pcall wrappers on `UIWidgetTemplateTextWithStateMixin:Setup`, `UIWidgetTemplateHorizontalCurrenciesMixin:Setup`, and `UIWidgetTemplateItemDisplayMixin:Setup` (`Blizzard_UIWidgetTemplateBase.lua:1638` arithmetic on secret-value dimensions). Each suppresses only secret-number errors and re-raises everything else; widget content is populated before the error point so visible state is intact. |
