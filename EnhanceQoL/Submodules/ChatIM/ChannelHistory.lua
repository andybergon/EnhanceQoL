...
function ChannelHistory:CreateDebugFrame()
	...
	-- Placeholder labels
	local leftLabel = f.left:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	leftLabel:SetPoint("TOPLEFT", f.left, "TOPLEFT", 10, -10)
	leftLabel:SetText("Scope/Characters (placeholder)")

	local midTitle = CreateFrame("Frame", nil, f.middle)
	midTitle:SetSize(220, 42)
	midTitle:SetPoint("TOP", f.middle, "TOP", 0, 35)
	midTitle.bg = midTitle:CreateTexture(nil, "BACKGROUND")
	midTitle.bg:SetAllPoints()
	midTitle.bg:SetAtlas("islands-queue-difficultyselector-backboard")
	midTitle.bg:SetAlpha(0.95)
	...
	local rightLabel = f.right:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
	rightLabel:SetPoint("TOPLEFT", f.right, "TOPLEFT", 10, -10)
	rightLabel:SetText("Log View (placeholder)")
