...
local function initUI()
	...
	addon.functions.InitDBValue("disableRightClickTargetingInCombat", false)
	addon.functions.InitDBValue("disableDoubleClickTargetingInCombat", false)
	addon.functions.InitDBValue("disableRightClickTargetingOutOfCombat", false)
...
local function findGossipSkipOption(options)
	...
	for _, v in pairs(options) do
		if v.name then
			local isRed = v.name:lower():match("^|cffff0000") ~= nil
			local stripped = v.name:gsub("|c%x%x%x%x%x%x%x%x", ""):gsub("|r", "")
			if isRed and stripped:match("^<.+>$") then
